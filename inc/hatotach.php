...
// 	echo 'enter simply_modify_orderitem func';
// 	echo "<pre>";
// 	print_r($data);
// 	echo "</pre>";
	//die();
...
function simply_func($data)
{
	...
	//     }
// 	if (isset($data['ORDERITEMS_SUBFORM']) && is_array($data['ORDERITEMS_SUBFORM'])) {
// 		$data['ORDERITEMS_SUBFORM'] = array_filter($data['ORDERITEMS_SUBFORM'], function($item) {
// 			return isset($item['PARTNAME']) && $item['PARTNAME'] === '900001';
// 		});

// 		// Optionally reindex the array to reset keys
// 		$data['ORDERITEMS_SUBFORM'] = array_values($data['ORDERITEMS_SUBFORM']);
// 	}
// 	
	if (isset($data['ORDERITEMS_SUBFORM']) && is_array($data['ORDERITEMS_SUBFORM'])) {
        // Loop through and filter items that are NOT WooCommerce products (e.g., shipping lines)
        $data['ORDERITEMS_SUBFORM'] = array_filter($data['ORDERITEMS_SUBFORM'], function($item) {

            // Check if this partname (SKU) exists as a WooCommerce product
            $product_id = wc_get_product_id_by_sku($item['PARTNAME']);
            return empty($product_id); // Keep only if it's NOT a valid product SKU
        });
...
function syncInventoryPriority()
{
    ...
    $response = WooAPI::instance()->makeRequest('GET', 'LOGPART?$select='.$data['select'].'&$filter=SHOWINWEB eq \'Y\' &' . $data['expand'], [], true);
...
function simply_after_receipt_func($array)
{
    ...
    if ($res['http_code'] <= 201) {
        ...
        if (isset($response_data['message'])) {
            ...
            $msg = "Failed close receipt.";
        }
		$order = wc_get_order($order_id);
		$order->update_meta_data('priority_recipe_status', $msg);
		$order->save();
        //wp_mail('elisheva.g@simplyct.co.il','failed close receipt', $msg.' error code'.$res['code']);
		return;
    }
    curl_close($curl);
...
//on update product, set product status to publish if show in web
