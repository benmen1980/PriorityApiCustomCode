...
function sync_product_attachemtns()
{
	...
	*/
	...
	/* first get only the items */
	$url = 'LOGPART?$filter=UDATE ge ' . urlencode($bod) . ' and EXTFILEFLAG eq \'Y\'';
	$response1 = WooAPI::instance()->makeRequest('GET',$url, [], WooAPI::instance()->option('log_attachments_priority', true));
	$response_data1 = json_decode($response1['body_raw'], true);
	foreach ($response_data1['value'] as  $partname) {
		$url = 'LOGPART?$filter=PARTNAME eq \''.$partname['PARTNAME'].'\' &$select=' . $search_field_select . '&$expand=PARTEXTFILE_SUBFORM($select=EXTFILENAME,EXTFILEDES,SUFFIX;$filter=SUFFIX eq \'png\' or SUFFIX eq \'jpeg\' or SUFFIX eq \'jpg\')';
		...
		foreach ($response_data2['value'] as  $item) {
			...
			foreach ($item['PARTEXTFILE_SUBFORM'] as $index =>$attachment) {
				...
				if (!empty($file_path)) {
					...
					$file = WooAPI::instance()->save_uri_as_image($product_full_url, $file_name);
					$attach_id = $file[0];
					$file_name = $file[1];
					if ($attach_id == null) {
						continue;
